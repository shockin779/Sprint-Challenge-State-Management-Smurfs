...
- [x] What problem does the context API help solve?
  ...
  context api allows us to access our Application level state from anywhere without having to do prop drilling. This allows us to more easily access state and update it where needed.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  Actions tell the application what actions it can perform.
  Reducers are pure functions that take the current state and the action you want to perform, then return a new state and tell the store what to do.
  The store is the object that holds the state of the application
  It is known as a single source of truth because all of your application state lives in one place and easier to manage that way. That way, there is only one place where the state can be.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is state that needs to be tracked and used throughout your application.
  Component state is state that only the component it is on needs to know about. A great thing for application state is updating the text on a form before submitting it.
...
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
  ...
  Redux, because it feels more natural to me than context api. It seems to be very powerful, especially when adding asynchronous ability to it.
